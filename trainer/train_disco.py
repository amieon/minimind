...
def grpo_train_epoch(epoch, loader, iters, rollout_engine, ref_model, reward_model, start_step=0, wandb=None,
    ...
    for step, batch in enumerate(loader, start=start_step + 1):
        ...
        # rewards: [B * G]
        # MiniMind reward 不是 binary，这里只是弱化版 DisCO
        ...
        # sequence average log likelihood score
        seq_logp = (per_token_logps * mask).sum(dim=1) / token_count
        seq_old_logp = (old_per_token_logps * mask).sum(dim=1) / token_count

        # log-L score
        score = seq_logp

        # 或者弱化版 L-ratio score
        # score = torch.exp(seq_logp - seq_old_logp)

        if pos_mask.any() and neg_mask.any():
            pos_score = score[pos_mask]
            neg_score = score[neg_mask]

            disc_obj = pos_score.mean() - neg_score.mean()
        else:
            # 这一批全正或全负时，判别式目标没法算
            # 简单处理，不做 DisCO 更新，只靠 KL 或者 fallback 到原 GRPO loss
            disc_obj = torch.tensor(0.0, device=args.device)
        ...
        kl_value = seq_kl.mean()

        constraint_violation = torch.clamp(kl_value - args.kl_target, min=0.0)
        kl_penalty = args.penalty_coef * constraint_violation.pow(2)

        loss = -disc_obj + kl_penalty
...
if __name__ == "__main__":
    ...
    parser.add_argument("--kl_target", type=float, default=0.05)
    parser.add_argument("--penalty_coef", type=float, default=50.0)
