...
def grpo_train_epoch(epoch, loader, iters, rollout_engine, ref_model, reward_model, start_step=0, wandb=None,
    ...
    for step, batch in enumerate(loader, start=start_step + 1):
        ...
        # Dr.GRPO: 不按每条 response 自己的长度做平均
        seq_loss = (per_token_loss * completion_mask).sum(dim=1)

        # 为了让 loss 数值不要随 max_gen_len 爆炸，可以除以一个固定常数
        # 最简单用 args.max_gen_len
        policy_loss = (seq_loss / args.max_gen_len).mean()
