...
def grpo_train_epoch(epoch, loader, iters, rollout_engine, ref_model, reward_model, start_step=0, wandb=None,
    ...
    for step, batch in enumerate(loader, start=start_step + 1):
        ...
        mean_group = grouped_rewards.mean(dim=1, keepdim=True)
        std_group = grouped_rewards.std(dim=1, keepdim=True, unbiased=False).clamp_min(1e-4)

        # 原始 GRPO advantage
        advantages_group = (grouped_rewards - mean_group) / std_group

        # MiniMind 没有 binary correct：
        # 用 reward > group mean 近似“成功”
        grouped_correct = (grouped_rewards > mean_group).float()  # [B, G]

        success_rate = grouped_correct.mean(dim=1, keepdim=True)  # [B, 1]

        # F-GRPO focal weight
        focal_weight = (1.0 - success_rate).pow(args.f_grpo_gamma)  # [B, 1]

        # 乘到同一个 prompt 的所有 generations 上
        advantages_group = advantages_group * focal_weight
