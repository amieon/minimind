...
def gmpo_policy_loss(
    per_token_logps: torch.Tensor,        # [N, R]
    old_per_token_logps: torch.Tensor,    # [N, R]
    advantages: torch.Tensor,             # [N]
    completion_mask: torch.Tensor,        # [N, R]
    ...
    N = batch_size * num_generations
    R = max completion length

    核心思想：
    1. 不再对 token-level ratio * advantage 做算术平均。
    2. 对 token log-ratio 做有符号 clipping。
    3. 对有效 token 的 clipped log-ratio 取平均，再 exp，得到 sequence-level geometric mean ratio。
    4. loss = - advantage * seq_ratio + KL penalty
    """

    mask = completion_mask.float()
    token_count = mask.sum(dim=1).clamp_min(1.0)  # [N]

    log_ratio = per_token_logps - old_per_token_logps  # [N, R]

    # sign(A): A > 0 时希望提高概率；A < 0 时希望降低概率
    adv_sign = torch.where(
        advantages >= 0,
        torch.ones_like(advantages),
        -torch.ones_like(advantages),
    )  # [N]

    signed_log_ratio = adv_sign.unsqueeze(1) * log_ratio  # [N, R]

    # GMPO token-level signed clipping
    # 对 A>0：限制 log_ratio 上界
    # 对 A<0：限制 -log_ratio 上界，也就是限制 log_ratio 下界
    clipped_signed_log_ratio = torch.clamp(
        signed_log_ratio,
        min=-epsilon,
        max=epsilon,
    )

    # 论文伪代码中是 min(signed, clipped_signed)
    signed_log_ratio_for_obj = torch.min(
        signed_log_ratio,
        clipped_signed_log_ratio,
    )

    clipped_log_ratio = adv_sign.unsqueeze(1) * signed_log_ratio_for_obj

    # 几何平均 ratio:
    # exp(mean(log ratio))
    seq_log_ratio = (clipped_log_ratio * mask).sum(dim=1) / token_count
    seq_ratio = torch.exp(seq_log_ratio)  # [N]

    # policy objective
    policy_loss_per_seq = -advantages * seq_ratio  # [N]

    if ref_per_token_logps is not None and beta > 0:
        kl_div = ref_per_token_logps - per_token_logps
        per_token_kl = torch.exp(kl_div) - kl_div - 1
        seq_kl = (per_token_kl * mask).sum(dim=1) / token_count
        policy_loss_per_seq = policy_loss_per_seq + beta * seq_kl

    return policy_loss_per_seq.mean()
